...
*/
 ...
 /*@bgen(jjtree) declarador_variable_id */
  ...
  if (usaInterfaz)
  {
    ...
    jjtn000.value = t.image;
        ...
        {
                gestorTSIntermedia.insertar(lexema, new Atributos(lexema,"",""+aliasCounter));
                aliasCounter++;
        }
        else
        {
 ...
 /*@bgen(jjtree) #asignacion( 2) */
  ...
  if (t == "=")
    {
      if (sim2.comienzo == "post"){
        aux = nuevaTemp();
        sim.codigo += aux + ":=" + sim2.lugar + "\u005cn";
    sim.codigo += sim.lugar + ":=" + aux + "\u005cn";
  ...
  if (usaInterfaz)
  {
    ...
    aux = nuevaTemp();
    sim.codigo += aux + ":=" + sim.lugar + "+" + 1;
    sim.codigo += sim.lugar + ":=" + aux + "\u005cn" + "\u005cn";
    {if (true) return sim;}
    throw new Error("Missing return statement in function");
  }
  ...
  if (usaInterfaz)
  {
    ...
    while (true) {
      ...
      if (sim.verdadera == "argumentos")
          sim2.codigo +="call " + sim1.lugar + ", " + sim2.lugar + "\u005cn";
      else
      {
        System.out.println("ESTOY AQUI\u005cn");
        sim3.lugar += sim2.lugar;
        sim3.codigo += sim2.codigo;
      }
    }
    sim2 = sim3;
    sim.lugar = sim1.lugar + sim2.lugar;
    sim.codigo = sim1.codigo + sim2.codigo;
    if (!sim2.lugar.equals(""))
    {
      aux = sim.lugar;
      sim.lugar = nuevaTemp();
      sim.codigo += sim.lugar + ":=" + aux + "\u005cn";
  ...
  if (usaInterfaz)
  {
    ...
    while (true) {
      ...
      sim2 = expresion();
    sim.codigo += sim2.codigo;
    sim.codigo += "param " + sim2.lugar + "\u005cn";
    numeroarg++;
    }
    sim.lugar = "argumentos"; //FIXME es una cutrada
    sim.verdadera = "" + numeroarg;
    {if (true) return sim;}
    throw new Error("Missing return statement in function");
  }
